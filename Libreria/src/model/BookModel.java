...
public class BookModel implements CRUD {
    ...
    public Book consultAuthor(int id_author) {
        ...
        try {
            //3. Sentencia SQL
            String sql = "SELECT book.id, book.title, book.id_author,author.name, book.yearPublication, book.price\n" +
                    "FROM book\n" +
                    "LEFT JOIN author ON book.id_author = ? ;";

            //4. Preparamos el statement
            PreparedStatement objPrepare = objConnection.prepareStatement(sql);

            //5. Darle valor al paremetro del query
            objPrepare.setInt(1, id_author);
